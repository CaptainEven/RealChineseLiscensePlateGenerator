...
class LQGTDataset(data.Dataset):
    ...
    def _init_lmdb(self):
        ...
        self.GT_env = lmdb.open(self.opt["dataroot_GT"],
                                ...
                                readahead=False,
                                meminit=False, )
        self.LR_env = lmdb.open(self.opt["dataroot_LQ"],
                                readonly=True,
                                lock=False,
                                readahead=False,
                                meminit=False, )
    ...
    def __getitem__(self, idx):
        ...
        if self.opt["data_type"] == "lmdb":
            resolution = [int(s) for s in self.GT_sizes[idx].split("_")]
        else:
            resolution = None
        ...
        # if self.opt["phase"] == "train":
        #     H, W, C = img_LR.shape
        #     assert LR_size[0] == GT_size[0] // scale, "GT size does not match LR size"
        #     assert LR_size[1] == GT_size[1] // scale, "GT size does not match LR size"
        #
        #     # randomly crop
        ...
        # if LR_size[1] < H and LR_size[0] < W:
        #     # center crop
        #     rnd_h = H // 2 - LR_size // 2
        #     rnd_w = W // 2 - LR_size // 2
        #     img_LR = img_LR[rnd_h: rnd_h + LR_size, rnd_w: rnd_w + LR_size, :]
