...
class LQDataset(data.Dataset):
    ...
    def _init_lmdb(self):
        ...
        self.LR_env = lmdb.open(self.opt["dataroot_LR"],
                                readonly=True,
                                lock=False,
                                readahead=False,
                                meminit=False, )
    ...
    def __getitem__(self, idx):
        ...
        # get LR image
        LR_path = self.LQ_paths[idx]
        if self.opt["data_type"] == "lmdb":
            resolution = [int(s) for s in self.LR_sizes[idx].split("_")]
        else:
            resolution = None
        img_LR = util.read_img(self.LR_env, LR_path, resolution)  # return: Numpy float32, HWC, BGR, [0,1]
