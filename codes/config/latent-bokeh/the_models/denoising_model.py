...
class DenoisingModel(BaseModel):
    ...
    def feed_data(self, state, LQ, GT=None, src_lens=None, tgt_lens=None, alpha=None):
    ...
    def optimize_parameters(self, step, timesteps, sde=None):
        ...
        lens_info = [self.src_lens, self.tgt_lens]

        # Get noise and score
        noise = sde.noise_fn(self.state, timesteps.squeeze(), lens_info=lens_info)
    ...
    def test(self, sde=None, perform_ode=False, save_states=False):
        ...
        with torch.no_grad():
            ...
            if not perform_ode:
                # for SDE
                self.output = sde.reverse_sde(self.state, save_states=save_states, lens_info=lens_info)
            else:
