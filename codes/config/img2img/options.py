...
from codes.sdeutils.file_utils import OrderedYaml
...
def parse(opt_path, is_train=True):
    ...
    if not os.path.isfile(opt_path):
        print("[Info]: invalid opt path: {:s}".format(opt_path))
...
def check_resume(opt, resume_iter):
    ...
    if opt["path"]["resume_state"]:
        ...
        opt["path"]["pretrain_model_G"] = osp.join(
            opt["path"]["the_models"], "{}_G.pth".format(resume_iter)
        )
        logger.info("Set [pretrain_model_G] to " + opt["path"]["pretrain_model_G"])
        if "gan" in opt["model"]:
            opt["path"]["pretrain_model_D"] = osp.join(
                opt["path"]["the_models"], "{}_D.pth".format(resume_iter)
            )
