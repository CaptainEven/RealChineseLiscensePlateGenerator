...
import codes.config.img2img.the_models.lr_scheduler as lr_scheduler
import codes.config.img2img.the_models.networks as networks
from codes.config.img2img.the_models.optimizer import Lion

from codes.config.img2img.the_models.modules.loss import MatchingLoss
...
class DenoisingModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if train_opt['optimizer'] == 'Adam':
                train_opt["lr_G"] = float(train_opt["lr_G"])
                print("--> LR: ", train_opt["lr_G"])
            ...
            if train_opt["lr_scheme"] == "MultiStepLR":
                for optimizer in self.optimizers:
                    self.schedulers.append(lr_scheduler.MultiStepLR_Restart(optimizer,
                                                                            train_opt["lr_steps"],
    ...
    def feed_data(self, noisy_state, LQ, GT=None):
        """
        @param noisy_state:
        @param LQ:
        @param GT:
        @return:
        """
        self.state = noisy_state.to(self.device)  # noisy_state
    ...
    def optimize_parameters(self, step, time_steps, sde=None):
        ...
        # Get noise and score
        noise = sde.noise_fn(self.state, time_steps.squeeze())
        score = sde.get_score_from_noise(noise, time_steps)

        # Learning the maximum likelihood objective for state x_{t-1}
        xt_1_expection = sde.reverse_sde_step_mean(self.state, score, time_steps)
