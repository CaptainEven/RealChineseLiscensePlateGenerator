...
def main():
    ...
    for phase, dataset_opt in opt["datasets"].items():
        if phase == "train":
            ...
            if opt["dist"]:
                train_sampler = DistIterSampler(train_set,
                                                world_size,
                                                rank,
                                                dataset_ratio)
    ...
    #### resume training
    if resume_state:
        logger.info("Resuming training from epoch: {}, iter: {}."
    ...
    print("--> val freq:", opt["train"]["val_freq"])

    for epoch in range(start_epoch, total_epochs + 1):
        if opt["dist"]:
            train_sampler.set_epoch(epoch)
        for iter_i, train_data in enumerate(train_loader):
            current_step += 1
