...
def generate_one_plate(generator,
    ...
    """
    if plate_layers == 0:
        img_name = str(plate_number) + "_" + str(plate_color) + "_single"
    else:
        img_name = str(plate_number) + "_" + str(plate_color) + "_double"
    img = generator.generate_plate_special(plate_number, plate_color, plate_layers)
    save_dir = os.path.join(save_dir, img_name + ".jpg")
    cv2.imwrite(save_dir, img)
    print("--> {:s} saved".format(save_dir))
